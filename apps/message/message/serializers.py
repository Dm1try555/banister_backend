...
class PrivateChatCreateSerializer(serializers.ModelSerializer):
    participant_id = serializers.IntegerField(
        write_only=True,
        help_text="ID участника для приватного чата"
    )
    
    class Meta:
        model = Chat
        fields = ['type', 'participant_id']
    
    def validate(self, attrs):
        """Валидация для приватного чата"""
        if attrs.get('type') != 'private':
            raise serializers.ValidationError("Тип чата должен быть 'private'")
        
        participant_id = attrs.get('participant_id')
        if not participant_id:
            raise serializers.ValidationError("participant_id обязателен для приватного чата")
        ...
        try:
            ...
            raise serializers.ValidationError("Участник не найден")
