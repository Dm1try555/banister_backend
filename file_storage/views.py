        ...
        try:
            ...
            file_storage = FileStorage.objects.create(
                ...
                is_public=True
                        ...
                        # Always handle existing photos (delete old files and deactivate old records)
            ...
            # Determine if this is first upload or change
            is_first_upload = not ProfilePhoto.objects.filter(user=request.user, is_active=False).exists()
            
            return self.success_response(
                data=ProfilePhotoSerializer(profile_photo).data,
                message='Profile photo uploaded successfully' if is_first_upload else 'Profile photo changed successfully'
            )
